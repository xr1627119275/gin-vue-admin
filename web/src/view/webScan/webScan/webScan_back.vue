  ...
  <div>
      ...
      >
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
          <template #default="scope">
            <el-button type="primary" link icon="add" @click="runScan(scope.row)">扫描</el-button>
            <el-button type="primary" link icon="edit" class="table-button" @click="updateWebScanFunc(scope.row)">变更
            </el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
      ...
      <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
        <el-form-item label="HOST:" prop="host">
          <el-input v-model="formData.host" :clearable="true" placeholder="请输入HOST"/>
        </el-form-item>
      </el-form>
    </el-drawer>
...
import ScanModal from "@/view/webScan/webScan/ScanModal.vue";
...
const runScan = async (row) => {
  const {ID , host} = row
  const res = await service({
    url: '/w_scan/runScan',
    method: 'post',
    data: { ID: String(row.ID), host: row.host }
  })
  console.log(res)
  if (res.code === 0) {
    ElMessage({
...
const enterDialog = async () => {
  elFormRef.value?.validate(async (valid) => {
    ...
    switch (type.value) {
      case 'create':
        res = await createWebScan(formData.value)
        break
      case 'update':
        res = await updateWebScan(formData.value)
        break
      default:
        res = await createWebScan(formData.value)
        break
