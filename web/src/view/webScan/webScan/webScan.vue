...
<!--                <el-form-item label="设置扫描模式" title="设置扫描模式: ssh (default 'all')">-->
<!--                  <el-input v-model="formData.args.m" placeholder="设置扫描模式: ssh (default 'all')"></el-input>-->
<!--                </el-form-item>-->
<!--              </el-col>-->
              <el-col :span="16">
              ...
              <el-col :span="16">
                <el-form-item label="跳过主机IP">
                  <el-input v-model="formData.args.hn" placeholder="请输入跳过主机IP"></el-input>
                </el-form-item>
              </el-col>

              <el-col :span="8">
                <el-form-item label="跳过的端口">
                  <el-input v-model="formData.args.pn" placeholder="扫描时要跳过的端口,as: 445"></el-input>
                </el-form-item>
              </el-col>

              <el-col :span="24">
                <el-form-item label="新增端口">
                  <el-input v-model="formData.args.pa"
                            placeholder="新增需要扫描的端口,3389 (会在原有端口列表基础上,新增该端口)"></el-input>
                </el-form-item>
              </el-col>

              <el-col :span="24">
                <el-form-item label="设置域名">
                  <el-input v-model="formData.args.domain" placeholder="smb爆破模块时,设置域名"></el-input>
                </el-form-item>
              </el-col>

              <el-col :span="12">
                <el-form-item label="设置代理">
                  <el-input v-model="formData.args.proxy" placeholder="设置代理, http://127.0.0.1:8080"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="socks5代理">
                  <el-input v-model="formData.args.socks5" placeholder="指定socks5代理, socks5://127.0.0.1:1080"></el-input>
                </el-form-item>
              </el-col>


              <el-col :span="14">
                <el-form-item label="指定Url扫描">
                  <el-input type="textarea" v-model="formData.args.u" placeholder="指定Url扫描"
                            :autosize="{minRows: 5}"></el-input>
                </el-form-item>
              </el-col>

              <el-col :span="5">
                <el-form-item label="扫描线程数">
                  <el-input v-model="formData.args.t" placeholder="请输入扫描线程数"></el-input>
                </el-form-item>

                <el-form-item label="web访问超时时间">
                  <el-input v-model="formData.args.wt" placeholder="请输入 web访问超时时间"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="5">
                <el-form-item label="扫描超时时间">
                  <el-input v-model="formData.args.time" placeholder="请输入端口扫描超时时间"></el-input>
                </el-form-item>

                <el-form-item label="web poc 发包速率">
                  <el-input v-model="formData.args.num" placeholder="web poc 发包速率 (default 20)"></el-input>
                </el-form-item>
                <!--            <el-form-item label="扫描超时时间">-->
                <!--              <el-input v-model="formData.time" placeholder="请输入端口扫描超时时间"></el-input>-->
                <!--            </el-form-item>-->
              </el-col>
              <el-col :span="24">
                <el-form-item label="设置Cookie">
                  <el-input type="textarea" v-model="formData.args.cookie" placeholder="请输入Cookie"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="24">
                <el-form-item label="其他">
                  <el-checkbox-group v-model="formData.args.extra">
                    <el-checkbox label="nobr" :value="'nobr'">跳过sql、ftp、ssh等的密码爆破</el-checkbox>
                    <el-checkbox label="nopoc" :value="'nopoc'">跳过web poc扫描</el-checkbox>
                    <el-checkbox label="np" :value="'np'">跳过存活探测</el-checkbox>
                    <el-checkbox label="ping" :value="'ping'">使用ping代替icmp进行存活探测</el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
              </el-col>
            </el-row>
...
const formData = reactive({
  host: '',
  port: '21,22,80,81,135,139,443,445,1433,3306,5432,6379,7001,8000,8080,8089,9000,9200,11211,27017',
  args: {
    m: 'all',
    t: '600',
    time: '3',
...
const executeTest = async () => {
  ...
  timeId.value = setInterval(async () => {
    try {
      res = await findWebScan({ID: jobId })
      const result = res?.data?.result
      logs.value = result
      if (result.includes('扫描结束')) {
        const res = await parseScan({task_id: jobId, type: 'fscan'})
        console.log(res)
...
// 删除行
const detailInfo = async (row) => {
  const res = await parseScan({task_id: String(row.ID), type: 'fscan'})
  console.log(res)
  debugger
