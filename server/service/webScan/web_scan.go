...
func (w_scanService *WebScanService) RunScan(params webScanReq.RunWebScanParam, runType string) (err error) {
	...
	scanInfo, err := w_scanService.GetWebScan(params.WebScan.GVA_MODEL_UUID.ID.String())
	if err != nil {
		return err
	}
	for scanner.Scan() {
...
func (w_scanService *WebScanService) RunAFrogScan(params webScanReq.RunWebScanParam) (err error, saveInfo webScan.WebScan) {
	var cmd *exec.Cmd
	var execFileName = "afrog"
	//var runType = ""
	currentTime := time.Now()
	timestamp := currentTime.Unix()
	timestampStr := fmt.Sprintf("%d", timestamp)
	...
	args = append(args, "-ja", outFileName)
